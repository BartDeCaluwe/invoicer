defmodule InvoicerWeb.Endpoint do
  ...
  socket("/socket", InvoicerWeb.Socket)
  ...
  plug(InvoicerHealth.Router)
  plug(InvoicerGraphQL.Router)
  plug(:halt_if_sent)
  ...
  def init(_key, config) do
    if config[:load_from_system_env] do
      port = Application.get_env(:invoicer, __MODULE__)[:http][:port] || raise "expected the PORT environment variable to be set"
  ...
  defp ping(%{request_path: "/ping"} = conn, _opts) do
    version = Application.get_env(:invoicer, :version)
  ...
  defp canonical_host(conn, _opts) do
    opts = PlugCanonicalHost.init(canonical_host: Application.get_env(:invoicer, :canonical_host))
  ...
  defp force_ssl(conn, _opts) do
    if Application.get_env(:invoicer, :force_ssl) do
  ...
  defp cors(conn, _opts) do
    opts = Corsica.init(Application.get_env(:invoicer, Corsica))

    Corsica.call(conn, opts)
  end

  defp basic_auth(conn, _opts) do
    basic_auth_config = Application.get_env(:invoicer, :basic_auth)
